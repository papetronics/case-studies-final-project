...
As we suspected, there is a Pareto frontier between these two objectives. We can see that full game performance generally increases linearly with single-turn performance.
...
Lastly, figure \ref{fig:action_representation} shows results where we tested our hypothesis that a 32-way categorical would prove beneficial to complex actions that required specific combinations of dice to be held (see Section \ref{sec:rolling-action}).
...
\begin{table*}[t]
    ...
    \begin{tabular}{lcccccc}
        ...
        PPO ($\lambda=0.3$, $k=5$) & 50k games                & $<X>$               & $<Y>$            & $<X>$                    & $<Y>$                      & $<Z>$                   \\
