...
These results are best reported in Table~\ref{tab:final-results}.
...
as seen in Figure~\ref{fig:single-vs-full-game}, and found that with certain hyperparameters, we
...
as shown in Table~\ref{tab:ablation-results}.
As seen in Table~\ref{tab:ablation-results}, the state and action encodings played a significant role; specifically providing (rather than forcing the network to learn) some easily calculable features improved learning.
Categorical action distributions outperformed Gaussian ones, as seen in Table~\ref{tab:ablation-results}, allowing the network to better model compound actions.
The choice of activation function also mattered; with <BLAH outperforming BLAH>, likely due to better gradient flow.
Adding LayerNorm layers improved stability and final performance as well, as shown in Table~\ref{tab:ablation-results}.
Lastly, we found diminishing returns for model size; larger models improved performance up to a point, but after a certain size, gains were minimal, as shown in Figure~\ref{fig:model-size-ablation}.
...
However, they did not lead to significant improvement, as shown in Table~\ref{tab:ablation-results};
...
Entropy regularization played a huge role in stabilizing training and encouraging exploration, best seen in Figure~\ref{fig:entropy-ablation};
...
From the per-turn statistics in Figure~\ref{fig:turn-by-turn-stats}, we can infer a typical game,
...
We see evidence of this when comparing the category mean scores against DP in Table~\ref{tab:final-results}.
...
Reward shaping definitely helped the agent learn the bonus more quickly, but had its limits, as seen in Table~\ref{tab:ablation-results}.
...
It's worth restating that, since we are using a learned potential function, we do break the theoretical guarantees of potential-based reward shaping \cite{ng1999policyinvariance},
