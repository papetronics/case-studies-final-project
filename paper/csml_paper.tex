...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
      node distance=1cm,
      block/.style={rectangle, draw, minimum width=2.5cm, minimum height=0.6cm, align=center, fill=blue!10},
      head/.style={rectangle, draw, minimum width=2.5cm, minimum height=0.6cm, align=center, fill=green!10},
      output/.style={rectangle, draw, minimum width=2.5cm, minimum height=0.6cm, align=center, fill=orange!10},
    ]

    % Input
    \node[block] (input) {State Features $\phi(\mathbf{s})$};

    % Trunk blocks
    \node[block, below of=input] (trunk1) {Block 1};
    \node[block, below of=trunk1] (trunk2) {Block 2};
    \node[block, below of=trunk2] (trunk3) {Block 3};

    % Split into three heads
    \node[head, below left of=trunk3, xshift=-2cm, yshift=-1.5cm] (roll_head) {Roll Head Block};
    \node[head, below of=trunk3, yshift=-1.5cm] (score_head) {Score Head Block};
    \node[head, below right of=trunk3, xshift=2cm, yshift=-1.5cm] (value_head) {Value Head Block};

    % Outputs
    \node[output, below of=roll_head] (roll_out) {Roll Action\\(Bernoulli/Categorical)};
    \node[output, below of=score_head] (score_out) {Score Action\\(Categorical)};
    \node[output, below of=value_head] (value_out) {Value Estimate\\(Scalar)};

    % Main trunk arrows
    \draw[->, thick] (input) -- (trunk1);
    \draw[->, thick] (trunk1) -- (trunk2);
    \draw[->, thick] (trunk2) -- (trunk3);

    % Split to heads
    \draw[->, thick] (trunk3) -- (roll_head);
    \draw[->, thick] (trunk3) -- (score_head);
    \draw[->, thick] (trunk3) -- (value_head);

    % Head to output arrows
    \draw[->, thick] (roll_head) -- (roll_out);
    \draw[->, thick] (score_head) -- (score_out);
    \draw[->, thick] (value_head) -- (value_out);
