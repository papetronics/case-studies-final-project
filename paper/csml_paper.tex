...
Each of these networks have a fully connected hidden layer before the final output layer.

In the rolling action head, we use either 6 outputs with sigmoid activations for Bernoulli representation, or 32 outputs with softmax activations for the categorical representation.
...
We also inject an entropy bonus into the loss function to encourage exploration \cite{williams-peng-1991-function-optimization}, these held
...
We first implement the REINFORCE algorithm \cite{williams-1992-reinforce} with baseline for single-turn optimization,
then attempt to extend it to full-game optimization. Here, the baseline is the output of the value head, $V_{\phi}(\mathbf{s})$.
...
\subsubsection{TD}
Temporal Difference differs from REINFORCE by calculating the TD error, which is the difference between the estimated value of the
current state and the discounted value of the next state plus rewards earned \cite{sutton-1988-temporal-differences}.
By doing so, TD methods reduce variance at the cost of introducing some bias. TD also has the benefit of being an online method;
we can learn from every step rather than waiting until the end of an episode, which greatly improves sample efficiency.
