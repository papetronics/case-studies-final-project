...
and explicit Kullback-Leibler (KL) divergence control to dramatically reduce variance and ensure stable updates.

In long episodic games, the choice of return calculation affects sample efficiency, bias, and variance. REINFORCE Monte-Carlo (MC) returns $G_{t}^{MC}$ use a summation over the full series of rewards until the end of the episode.
This approach is unbiased but has high variance.
In contrast, Temporal Difference methods use a "bootstraped estimate" of future rewards to reduce variance.
Essentially, they only consider received rewards $R$ in a specific time window, and use an estimate from the value function $V(S_{t+1})$ for future rewards beyond that window.
The TD erorr is the difference between this sum and the value estimate for the current state $V(S_t)$; and can be used as the advantage estimate for policy gradient methods \cite{sutton-1988-temporal-differences}.
These estimates are biased (since they rely on future value estimates to be accurate), but they have much lower variance than full-episode returns.
...
For example, n-step returns $G_{t}^{TD(n)}$ \cite{sutton-2018-reinforcement-book} interpolate between MC and single-step TD returns, allowing us to define a time horizon $n$ over which to sum rewards before bootstrapping, this lets us manually control the bias-variance tradeoff.
An improvement on n-step is this is the $TD(\lambda)$ algorithm \cite{sutton-2018-reinforcement-book}, which uses an exponentially weighted average of n-step returns, effectively blending multiple time horizons intoa single estimate controlled by $\lambda$.

Actor-Critic \cite{konda-1999-actorcritic} and PPO \cite{schulman-2017-ppo} can use any of these TD-style return estimation methods, including the generalized advantage estimate (GAE) \cite{schulman-2016-gae}, which is a special formulation of $TD(\lambda)$.
...
\subsection{Entropy}
We also inject an entropy bonus into the loss function to encourage exploration \cite{williams-peng-1991-function-optimization}, these are held
...
Once the model has figured out how to play the game, it quickly converges on one of these, and won't explore other strategies since they often trade off short-term rewards for long-term gains.
...
\begin{align*}
  ...
  }_{\text{rolling action entropy}} \\[4pt]
    & -
  \underbrace{
...
\begin{align*}
  ...
  \|_2
    ...
    & -
  \underbrace{
  \overbrace{\beta}^{\text{weight}}
  \mathcal{H}\big[\pi_\theta(\cdot \mid s_t)\big]
  }_{\text{entropy bonus}} \\
\end{align*}
...
For \textit{Yahtzee}, we utilize an episodic, one-step TD(0) Advantage Actor-Critic (A2C) method \cite{sutton-2018-reinforcement-book}.
...
\begin{align*}
  ...
  +
  \overbrace{\gamma V_\phi(s_{t+1})}^{\text{discounted future}}
  -
...
Pure TD methods can be considered a form of estimated DP, which makes them potentially well suited for \textit{Yahtzee}, for which a DP solution is known to exist.

Other forms of TD returns, such as n-step TD and TD($\lambda$) can be used by replacing $\delta_t$ with the appropriate return estimate.

\subsubsection{PPO}
PPO improves on TD by utilizing a "surrogate" objective which clips large policy updates, improving training stability.
This allows for substantially larger batch sizes and learning rates \cite{schulman-2017-ppo}.
