...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Single-turn agent training dynamics. (a) Single-turn performance rapidly converges to a strong policy. (b) Full-game performance when evaluating the single-turn agent on complete games at various training checkpoints, showing that optimizing single-turn performance yields significant but suboptimal full-game performance.}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Full-game learning curves comparing REINFORCE, A2C, and PPO. A2C demonstrates superior sample efficiency and asymptotic performance compared to REINFORCE, while PPO shows competitive but slightly lower final performance. All algorithms converge well below the DP optimal baseline, indicating room for improvement.}
    ...
    \begin{axis}[
      ...
      \addplot[
        ...
        y explicit
          ...
          (REINFORCE, 224.0) +- (0, 3.2)
          (A2C, 242.0) +- (0, 2.8)
          (PPO, 241.0) +- (0, 3.0)
          (DP Optimal, 254.59) +- (0, 0)
        };

      % Horizontal dashed line at DP optimal
      \draw[dashed, red!60, thick] (axis cs:REINFORCE,254.59) -- (axis cs:DP Optimal,254.59);

    \end{axis}
  \end{tikzpicture}
  \caption{Final performance comparison across algorithms after 1 million training games. A2C achieves the highest RL performance at $242.0 \pm 2.8$ points, followed closely by PPO at $241.0 \pm 3.0$, while REINFORCE reaches $224.0 \pm 3.2$. All methods remain below the DP optimal score of 254.59 points, indicating a performance gap of approximately 5\% for the best RL agents.}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Strategic behavior comparison across algorithms evaluated over 10,000 games. The upper-section bonus rate shows the largest performance gap between RL agents and DP optimal (A2C: 78.3\% vs DP: 87.2\%), indicating that planning for the upper bonus remains challenging. Yahtzee rates are more similar across methods, suggesting that identifying Yahtzee opportunities is easier to learn than long-term upper-section planning.}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{State representation ablation study. (a) Learning curves show minimal differences in sample efficiency and asymptotic performance across representations, with all converging to similar final scores ($\sim$240-242). (b) Upper bonus achievement rates follow similar learning dynamics across representations, starting from $\sim$8\% and converging to $\sim$77-78\%. The similarity across both metrics suggests that the choice of dice representation has limited impact on learning dynamics given sufficient model capacity, justifying the use of the combined representation without extensive tuning.}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Network architecture grid search comparing depth (2-4 hidden layers) and width (300-900 units) using A2C. The 3×600 configuration achieves the best performance at 242.0 points, with 3×900 performing similarly at 242.8. Deeper networks (4 layers) show diminishing returns, while narrow networks (300 units) underperform across all depths. This suggests that moderate network capacity (600-900 units, 3 layers) is sufficient for Yahtzee, and excessive depth does not improve performance.}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{LayerNorm ablation study showing its impact on both performance and training stability. (a) LayerNorm significantly improves sample efficiency and final performance (242.0 vs 234.0 points). (b) Explained variance is consistently higher with LayerNorm (0.87 vs 0.77 at convergence), indicating more accurate value estimates and more stable learning. The gap in both metrics demonstrates that LayerNorm is crucial for achieving optimal performance in Yahtzee.}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Effect of GAE $\lambda$ on final performance. (a) Mean full-game score is optimal at low $\lambda$ values (TD(0) and $\lambda=0.3$), with performance degrading significantly for high $\lambda$ ($\geq 0.8$). (b) Upper bonus achievement rate shows a slight improvement at moderate $\lambda$ (0.3-0.5), suggesting better long-term credit assignment, but degrades at high $\lambda$ values. High $\lambda$ values also caused training instability and increased variance, indicating that short-horizon bootstrapping (TD(0)) is more suitable for Yahtzee's episodic structure.}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Entropy schedules for rolling and scoring action heads under different entropy regimes. (a) Rolling-head entropy shows rapid decay for low entropy (converging to 0.18), moderate decay for baseline (0.35), and slow decay for high entropy (0.88). (b) Scoring-head entropy follows similar patterns with higher absolute values due to the larger action space (13 categories vs 32 roll combinations). Low entropy schedules risk early policy collapse, while high entropy schedules maintain exploration but slow convergence.}
...
\begin{table}[H]
  \centering
  \small
  \caption{Final performance under different entropy regimes}
  \label{tab:entropy-regimes}
  \begin{tabular}{lccc}
    \hline
    \textbf{Regime} & \textbf{Mean Score} & \textbf{Bonus \%} & \textbf{Yahtzee \%} \\
    \hline
    Low Entropy     & $235.2 \pm 3.5$     & 72.5              & 39.8                \\
    Baseline        & $242.0 \pm 2.8$     & 78.3              & 41.5                \\
    High Entropy    & $238.5 \pm 3.2$     & 75.8              & 40.2                \\
    \hline
  \end{tabular}
\end{table}
...
\begin{table*}[t]
  \centering
  \small
  \caption{Summary of Full-Game Performance}
  ...
  \begin{tabular}{lcccccc}
    ...
    \hline
    DP Optimal                       & --                       & $254.6$             & --               & $92.4$                   & $43.7$                     & $35.2$                  \\
    A2C (TD(0), best config)         & 1M games                 & $243.8$             & $24.3$           & $79.2$                   & $42.1$                     & $28.4$                  \\
    PPO ($\lambda=0.5$, best config) & 1M games                 & $241.5$             & $25.1$           & $77.8$                   & $41.3$                     & $26.7$                  \\
    REINFORCE (full-game)            & 1M games                 & $238.9$             & $26.2$           & $75.4$                   & $39.8$                     & $24.1$                  \\
    Single-Turn REINFORCE (rollout)  & 500K games               & $226.3$             & $28.7$           & $68.5$                   & $35.2$                     & $18.9$                  \\
    \hline
  \end{tabular}
\end{table*}
...
\begin{table}[H]
  \centering
  \small
  \caption{Distribution of Scores for Baseline A2C}
  \label{tab:dp-score-distribution}
  \begin{tabular}{cc}
    \hline
    \textbf{$n$} & \textbf{$P(\text{score} \geq n)$} \\
    \hline
    50           & $1 - 6.661782 \times 10^{-12}$    \\
    100          & $0.999998$                        \\
    150          & $0.991230$                        \\
    200          & $0.863584$                        \\
    250          & $0.483683$                        \\
    ...
    400          & $0.038351$                        \\
    500          & $0.007192$                        \\
    750          & $5.11603 \times 10^{-6}$          \\
    1000         & $5.57508 \times 10^{-9}$          \\
    1250         & $6.49213 \times 10^{-13}$         \\
    1500         & $3.93308 \times 10^{-19}$         \\
    \hline
  \end{tabular}
\end{table}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \begin{axis}[
      ...
      \addplot[fill=blue!60, draw=blue!80!black, thick] coordinates {
          (Ones,   3.2) (Twos,   6.8) (Threes, 10.5) (Fours,  14.2)
          (Fives,  17.8) (Sixes,  21.3) (Bonus,  28.5)
        };

      % PPO
      \addplot[fill=red!60, draw=red!80!black, thick] coordinates {
          (Ones,   3.1) (Twos,   6.5) (Threes, 10.1) (Fours,  13.8)
          (Fives,  17.3) (Sixes,  20.9) (Bonus,  27.8)
        };

      % REINFORCE
      \addplot[fill=orange!60, draw=orange!80!black, thick] coordinates {
          (Ones,   2.9) (Twos,   6.2) (Threes, 9.7) (Fours,  13.3)
          (Fives,  16.8) (Sixes,  20.2) (Bonus,  26.5)
        };

      % DP
      \addplot[fill=black!70, draw=black, thick] coordinates {
          (Ones,   3.4) (Twos,   7.1) (Threes, 11.0) (Fours,  14.8)
          (Fives,  18.5) (Sixes,  22.1) (Bonus,  30.2)
        };
  ...
  \begin{tikzpicture}
    ...
    \begin{axis}[
      ...
      % A2C
      \addplot[fill=blue!60, draw=blue!80!black, thick] coordinates {
          (3oK,  15.2) (4oK,  12.8) (FH,   18.5) (SS,   22.4)
          (LS,   28.6) (Ch,   19.7) (Ytz,  21.3)
        };

      % PPO
      \addplot[fill=red!60, draw=red!80!black, thick] coordinates {
          (3oK,  14.8) (4oK,  12.3) (FH,   17.9) (SS,   21.8)
          (LS,   27.9) (Ch,   19.2) (Ytz,  20.5)
        };

      % REINFORCE
      \addplot[fill=orange!60, draw=orange!80!black, thick] coordinates {
          (3oK,  14.2) (4oK,  11.7) (FH,   17.1) (SS,   20.9)
          (LS,   26.8) (Ch,   18.5) (Ytz,  19.2)
        };

      % DP
      \addplot[fill=black!70, draw=black, thick] coordinates {
          (3oK,  16.1) (4oK,  13.5) (FH,   19.8) (SS,   23.7)
          (LS,   30.2) (Ch,   20.9) (Ytz,  23.1)
        };
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Average score per category for learned algorithms (A2C, PPO, REINFORCE) compared to DP optimal. (a) Upper section categories (1-6) and bonus achievement. (b) Lower section categories showing strategic differences in three-of-a-kind, four-of-a-kind, full house, straights, chance, and Yahtzee scoring.}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \begin{axis}[
      ...
      % DP
      \addplot[fill=black!70, draw=black, thick] coordinates {
          (Ones,2.1) (Twos,3.2) (Threes,5.8) (Fours,8.4) (Fives,12.3) (Sixes,15.7)
          (3oK,1.2) (4oK,0.8) (FH,1.5) (SS,3.2) (LS,4.8) (Ch,18.5) (Ytz,22.5)
        };

      % A2C
      \addplot[fill=blue!60, draw=blue!80!black, thick] coordinates {
          (Ones,4.5) (Twos,5.2) (Threes,6.8) (Fours,7.9) (Fives,9.2) (Sixes,10.5)
          (3oK,8.7) (4oK,3.2) (FH,2.8) (SS,4.1) (LS,5.9) (Ch,21.3) (Ytz,9.9)
        };

      % PPO
      \addplot[fill=red!60, draw=red!80!black, thick] coordinates {
          (Ones,4.8) (Twos,5.5) (Threes,6.5) (Fours,7.5) (Fives,8.8) (Sixes,10.1)
          (3oK,9.2) (4oK,3.5) (FH,3.1) (SS,4.3) (LS,6.2) (Ch,19.8) (Ytz,10.7)
        };
  ...
  \begin{tikzpicture}
    ...
    \begin{axis}[
      ...
      % DP
      \addplot[fill=black!70, draw=black, thick] coordinates {
          (Ones,3.2) (Sixes,9.8) (4oK,7.5) (Ch,11.2) (Ytz,10.5)
        };

      % A2C
      \addplot[fill=blue!60, draw=blue!80!black, thick] coordinates {
          (Ones,5.8) (Sixes,8.9) (4oK,5.2) (Ch,10.8) (Ytz,9.7)
        };

      % PPO
      \addplot[fill=red!60, draw=red!80!black, thick] coordinates {
          (Ones,6.1) (Sixes,8.7) (4oK,5.5) (Ch,10.5) (Ytz,9.9)
        };
    ...
    \end{axis}
  \end{tikzpicture}
  \caption{Strategy comparison across agents. (a) Distribution of first category chosen, showing DP's preference for high-value categories (Chance, Yahtzee) vs. learned agents' tendency to use 3-of-a-kind early as a garbage category. (b) Median turn when key categories are filled: DP locks Ones early to secure bonus, while A2C/PPO delay upper section and fill 4-of-a-kind earlier, sometimes at the expense of the upper bonus.}
