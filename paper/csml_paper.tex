...
\begin{align*}
  \phi_{\mathrm{lockin}}(\mathbf{d}, \mathbf{c})   & \in \{0,1\}^{6},                                                                                      \\[4pt]
...
\begin{figure}[htb]
  ...
  \scalebox{0.70}{
    \begin{tikzpicture}[
      ...
      \node[norm, below of=trunk1_linear] (trunk1_ln) {LayerNorm};
      \node[activation, below of=trunk1_ln] (trunk1_swish) {Swish};

      % ===== TRUNK BLOCK 2 =====
      \node[linear, below=0.8cm of trunk1_swish] (trunk2_linear) {Linear};
      \node[norm, below of=trunk2_linear] (trunk2_ln) {LayerNorm};
      \node[activation, below of=trunk2_ln] (trunk2_swish) {Swish};

      % ===== TRUNK BLOCK 3 =====
      \node[linear, below=0.8cm of trunk2_swish] (trunk3_linear) {Linear};
      \node[norm, below of=trunk3_linear] (trunk3_ln) {LayerNorm};
      \node[activation, below of=trunk3_ln] (trunk3_swish) {Swish};

      % ===== ROLL HEAD =====
      \node[linear, below left=1.0cm and 1.4cm of trunk3_swish] (roll_linear) {Linear};
      \node[norm, below of=roll_linear] (roll_ln) {LayerNorm};
      \node[activation, below of=roll_ln] (roll_swish) {Swish};

      % ===== SCORE HEAD =====
      \node[linear, below=1.0cm of trunk3_swish] (score_linear) {Linear};
      \node[norm, below of=score_linear] (score_ln) {LayerNorm};
      \node[activation, below of=score_ln] (score_swish) {Swish};

      % ===== VALUE HEAD =====
      \node[linear, below right=1.0cm and 1.4cm of trunk3_swish] (value_linear) {Linear};
      \node[norm, below of=value_linear] (value_ln) {LayerNorm};
      \node[activation, below of=value_ln] (value_swish) {Swish};

      % ===== ROLL OUTPUT =====
      \node[linear, below=0.6cm of roll_swish] (roll_out_linear) {Linear};
      \node[activation, below of=roll_out_linear] (roll_out_softmax) {Softmax};
      \node[below=0.4cm of roll_out_softmax] (roll_out) {$\underbrace{a_{\mathrm{roll}}}_{\text{Rolling Action}}$};

      % ===== SCORE OUTPUT =====
      \node[linear, below=0.6cm of score_swish] (score_out_linear) {Linear};
      \node[activation, below of=score_out_linear] (score_out_softmax) {Softmax};
      \node[below=0.4cm of score_out_softmax] (score_out) {$\underbrace{a_{\mathrm{score}}}_{\text{Scoring Action}}$};

      % ===== VALUE OUTPUT =====
      \node[linear, below=0.6cm of value_swish] (value_out_linear) {Linear};
      \node[activation, below of=value_out_linear] (value_out_elu) {ELU};
      ...
      \begin{scope}[on background layer]
        ...
        \node[draw=blue!50, very thick, rectangle, rounded corners=8pt, fit={(trunk1_linear) (trunk1_ln) (trunk1_swish) (trunk2_linear) (trunk2_ln) (trunk2_swish) (trunk3_linear) (trunk3_ln) (trunk3_swish)}, inner sep=0.35cm, fill=blue!5] (trunk_box) {};

        % Roll head box (include output layers)
        \node[draw=green!50, very thick, rectangle, rounded corners=8pt, fit={(roll_linear) (roll_ln) (roll_swish) (roll_out_linear) (roll_out_softmax)}, inner sep=0.35cm, fill=green!5] (roll_box) {};

        % Score head box (include output layers)
        \node[draw=purple!50, very thick, rectangle, rounded corners=8pt, fit={(score_linear) (score_ln) (score_swish) (score_out_linear) (score_out_softmax)}, inner sep=0.35cm, fill=purple!5] (score_box) {};

        % Value head box (include output layers)
        \node[draw=orange!50, very thick, rectangle, rounded corners=8pt, fit={(value_linear) (value_ln) (value_swish) (value_out_linear) (value_out_elu)}, inner sep=0.35cm, fill=orange!5] (value_box) {};
      \end{scope}

      % Individual blocks with dashed grey outline (drawn on top)
      % Trunk Block 1
      \node[draw=gray!50, dashed, rectangle, rounded corners=5pt, fit={(trunk1_linear) (trunk1_ln) (trunk1_swish)}, inner sep=0.25cm, fill=none] {};

      % Trunk Block 2
      \node[draw=gray!50, dashed, rectangle, rounded corners=5pt, fit={(trunk2_linear) (trunk2_ln) (trunk2_swish)}, inner sep=0.25cm, fill=none] {};

      % Trunk Block 3
      \node[draw=gray!50, dashed, rectangle, rounded corners=5pt, fit={(trunk3_linear) (trunk3_ln) (trunk3_swish)}, inner sep=0.25cm, fill=none] {};

      % Roll Head inner block
      \node[draw=gray!50, dashed, rectangle, rounded corners=5pt, fit={(roll_linear) (roll_ln) (roll_swish)}, inner sep=0.25cm, fill=none] {};

      % Score Head inner block
      \node[draw=gray!50, dashed, rectangle, rounded corners=5pt, fit={(score_linear) (score_ln) (score_swish)}, inner sep=0.25cm, fill=none] {};

      % Value Head inner block
      \node[draw=gray!50, dashed, rectangle, rounded corners=5pt, fit={(value_linear) (value_ln) (value_swish)}, inner sep=0.25cm, fill=none] {};
      ...
      \draw[->, thick] (trunk1_linear) -- (trunk1_ln);
      \draw[->, thick] (trunk1_ln) -- (trunk1_swish);
      \draw[->, thick] (trunk1_swish) -- (trunk2_linear);
      \draw[->, thick] (trunk2_linear) -- (trunk2_ln);
      \draw[->, thick] (trunk2_ln) -- (trunk2_swish);
      \draw[->, thick] (trunk2_swish) -- (trunk3_linear);
      \draw[->, thick] (trunk3_linear) -- (trunk3_ln);
      \draw[->, thick] (trunk3_ln) -- (trunk3_swish);

      % Arrows - Split to heads
      \draw[->, thick] (trunk3_swish) -- (roll_linear);
      \draw[->, thick] (trunk3_swish) -- (score_linear);
      \draw[->, thick] (trunk3_swish) -- (value_linear);

      % Arrows - Roll head
      \draw[->, thick] (roll_linear) -- (roll_ln);
      \draw[->, thick] (roll_ln) -- (roll_swish);
      \draw[->, thick] (roll_swish) -- (roll_out_linear);
      \draw[->, thick] (roll_out_linear) -- (roll_out_softmax);
      \draw[->, thick] (roll_out_softmax) -- (roll_out);

      % Arrows - Score head
      \draw[->, thick] (score_linear) -- (score_ln);
      \draw[->, thick] (score_ln) -- (score_swish);
      \draw[->, thick] (score_swish) -- (score_out_linear);
      \draw[->, thick] (score_out_linear) -- (score_out_softmax);
      \draw[->, thick] (score_out_softmax) -- (score_out);

      % Arrows - Value head
      \draw[->, thick] (value_linear) -- (value_ln);
      \draw[->, thick] (value_ln) -- (value_swish);
      \draw[->, thick] (value_swish) -- (value_out_linear);
