...
Let us define a dice face counting function which we can use to simplify score calculations:
\begin{align*}
...
f_{13}(\mathbf{d})&= \mathbf{1}^\top \mathbf{d} \\[4pt]
\mathbf{f}(\mathbf{d}) &=
...
\mathbf{d}_{r+1} = \mathbf{d'} \cdot (1 - \mathbf{k}) + \mathbf{d} \cdot \mathbf{k}
$$
...
The state space for a solitaire game of Yahtzee can be described by the tuple of the current dice configuration,
...
solve directly \cite{yahtzee-tabular-approach}.
...
For a discrete control problem with a finite state size of $|\mathbf{S}|$, there are classical results \cite{horne-hush-1993-bounds-rnn-fsm} 
...
For a lower bound, \citet{hanin-2017-universal-approx-bounded-width-relu} shows any continuous, convex function 
