...
The basic block used throughout the network is shown in Figure~\ref{fig:basic-block}:

\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
      node distance=0.8cm,
      block/.style={rectangle, draw, minimum width=3cm, minimum height=0.6cm, align=center},
    ]
    ...
    % Linear layer
    \node[block, below=0.4cm of input] (linear) {Linear};

    % Layer Norm
    \node[block, below of=linear] (layernorm) {LayerNorm};

    % Swish activation
    \node[block, below of=layernorm] (swish) {Swish};

    % Output (outside the box)
    \node[below=0.4cm of swish] (output) {Output};

    % Surrounding box (drawn on background layer)
    \begin{scope}[on background layer]
      \node[draw=gray!50, thick, rectangle, rounded corners=5pt, fit={(linear) (layernorm) (swish)}, inner sep=0.3cm, fill=gray!5] {};
    \end{scope}

    % Arrows
    \draw[->, thick] (input) -- (linear);
    \draw[->, thick] (linear) -- (layernorm);
    \draw[->, thick] (layernorm) -- (swish);
    \draw[->, thick] (swish) -- (output);

  \end{tikzpicture}
  \caption{Basic building block}
  \label{fig:basic-block}
\end{figure}

The overall architecture is illustrated in Figure~\ref{fig:network-architecture}:
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    % ===== ROLL HEAD =====
    \node[linear, below left=0.8cm and 1.0cm of trunk3_swish] (roll_linear) {Linear};
    \node[norm, below of=roll_linear] (roll_ln) {LayerNorm};
    \node[activation, below of=roll_ln] (roll_swish) {Swish};

    % ===== SCORE HEAD =====
    \node[linear, below=0.8cm of trunk3_swish] (score_linear) {Linear};
    \node[norm, below of=score_linear] (score_ln) {LayerNorm};
    \node[activation, below of=score_ln] (score_swish) {Swish};

    % ===== VALUE HEAD =====
    \node[linear, below right=0.8cm and 1.0cm of trunk3_swish] (value_linear) {Linear};
    ...
    \begin{scope}[on background layer]
      % Trunk Block 1
      \node[draw=blue!50, thick, rectangle, rounded corners=5pt, fit={(trunk1_linear) (trunk1_ln) (trunk1_swish)}, inner sep=0.25cm, fill=blue!5] {};

      % Trunk Block 2
      \node[draw=blue!50, thick, rectangle, rounded corners=5pt, fit={(trunk2_linear) (trunk2_ln) (trunk2_swish)}, inner sep=0.25cm, fill=blue!5] {};

      % Trunk Block 3
      \node[draw=blue!50, thick, rectangle, rounded corners=5pt, fit={(trunk3_linear) (trunk3_ln) (trunk3_swish)}, inner sep=0.25cm, fill=blue!5] {};

      % Roll Head
      \node[draw=green!50, thick, rectangle, rounded corners=5pt, fit={(roll_linear) (roll_ln) (roll_swish)}, inner sep=0.25cm, fill=green!5] {};

      % Score Head
      \node[draw=green!50, thick, rectangle, rounded corners=5pt, fit={(score_linear) (score_ln) (score_swish)}, inner sep=0.25cm, fill=green!5] {};

      % Value Head
      \node[draw=green!50, thick, rectangle, rounded corners=5pt, fit={(value_linear) (value_ln) (value_swish)}, inner sep=0.25cm, fill=green!5] {};
    \end{scope}
