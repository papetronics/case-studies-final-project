...
It also requires a different action representation, since actions can no longer correspond to specific dice indices.
To mitigate this, our environment always sorts the dice in ascending order before passing them to the agent.

We attempted 3 different dice representations:
...
We also implemented a learned potential reward shaping mechanism to assess its impact on the model's final performance and ability to learn the bonus.
...
At the end of training, we run a final evaluation of 100,000 games to get a robust estimate of the agent's performance.