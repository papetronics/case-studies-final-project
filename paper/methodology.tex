...
$$
This is trained using $L_2$ loss, with weight $\beta_{\mathrm{regression}}$:
$$
    \mathcal{L}_{\mathrm{upper}}(\theta) = \biggl\|\hat{U}_\theta(s) - U_{\mathrm{norm}}\biggr\|_2^2
$$
...
\subsection{Auxilliary Losses}
For all algorithms, we have auxilliary for both the shaping head and for entropy:
\begin{align}
    \mathcal{L}_{\mathrm{aux}}(\theta, \phi)
    = &
    \underbrace{
        \overbrace{\beta_{\mathrm{regression}}}^{\text{weight}}
        \,
        L_{\mathrm{regression}}(\theta)
    }_{\text{upper bonus regression loss}} \nonumber \\
      & +
    \mathcal{L}_{\mathrm{entropy}}(\theta)
\end{align}
